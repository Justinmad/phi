...
- 多测试场景
    ...
    - 内网环境测试
        ...
        - 使用AB进行压力测试，100个并发，发起100000次请求，使worker进程cpu达到100%，启用-k
            - 原生的nginx反向代理：35K T/S
            - 开启router功能后测试结果：22K T/S
            - 开启dynamic ups和balancer之后： 16K T/S
            - 内存占用250M左右
            - <font color=red>测试结果和上面是相似的，在一台多核心的机器上做反向代理时，网络带宽总是会先于CPU满载，此时本项目与原生转发差别是几乎可以忽略的，而在只考虑CPU负载的情况下，相较于nginx的原生代理，本项目总是会有50%的性能损耗。</font>
        - 测试在保持大量并发连接数下的系统稳定性：
            - TODO
