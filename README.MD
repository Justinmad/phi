...
- 多测试场景
    - Local环境，所有相关应用都部署在同一机器，相当于忽略网络开销
        ...
        - 使用AB进行压力测试，20个并发，发起1000000次请求，使worker进程cpu达到100%
            ...
            - 多核心下cpu不会也不应该成为系统性能瓶颈
    - 内网环境测试
        - 测试机器配置
            - 三台 KVM CPU:2C RAM:2G 千兆网卡
            - 一台部署PHI应用，一台部署NGINX静态文件服务，一台压测
        - 使用AB进行压力测试，100个并发，发起100000次请求，使worker进程cpu达到100%
            - 原生的nginx反向代理：35K T/S
            - 开启router功能后测试结果：22K T/S
            - 开启dynamic ups和balancer之后： 16K T/S
            - 内存占用250M左右
            - 测试结果和上面是相似的，理论上1KB的文件访问，千兆网卡能达到80K+的TPS，在一台16核心的机器上，网络带宽应该会首先被打满，此时本项目与原生转发差别是几乎可以忽略的。
    - 测试在保持大量并发连接数下的系统稳定性：
        - TODO
